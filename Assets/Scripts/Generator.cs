...
public class Generator : MonoBehaviour {
    ...
    public GameObject refFocusStatusControl;
	...
	float _totalTimeIteration = 0f;
	float _totalFocusTime = 0f;

	MeshFilter _filter;
    
    FocusStatusControl focusstatus;
	...
	void Start () {
		focusstatus = refFocusStatusControl.GetComponent<FocusStatusControl>();
  ...
  void Update () {
		float rate = (float)(focusstatus.getvalue() - 5) * 0.5f;
		_totalFocusTime += Time.deltaTime * rate;
		_totalFocusTime = Mathf.Max(0f, _totalFocusTime);

		// we check if we need to run a new iteration 
		if (_totalFocusTime > _timeBetweenIterations + _totalTimeIteration) {
			_totalTimeIteration = _totalFocusTime;
				...
				if (_activeAttractors.Count != 0) {
					...
					foreach (Branch b in _branches) {
						...
						if (b._attractors.Count > 0) {
							...
							nb._timeformed = _totalFocusTime;
	...
	void ToMesh (float rate) {
		...
		for (int i = 0; i < _branches.Count; i++) {
			...
			float value = (_totalFocusTime - b._timeformed) / _timeBetweenIterations;
			...
			for (int s = 0; s < _radialSubdivisions; s++) {
				...
				pos += b._start + (b._end - b._start) * value;
		...
		// Debug.Log("_branches.Count: " + _branches.Count);
		// Debug.Log("validbranchnum.Count: " + validbranchnum);


		// faces construction; this is done in another loop because we need the parent vertices to be computed
		// rely on the fact that newly formed is at the end of list
		for (int i = 0; i < validbranchnum; i++) {
			Branch b = _branches[i];
		...
		int[] subtriangles = new int[validbranchnum * _radialSubdivisions * 6];
		for (int s = 0; s < subtriangles.GetLength(0); s++)
		{
			subtriangles[s] = triangles[s];
		}
		treeMesh.vertices = vertices;//.GetRangeake(validbranchnum);
		treeMesh.triangles = subtriangles;//.GetRange(0, validbranchnum * 6);
